...
const CACHE_TTL = 10 * 60 * 1000; // 10 分鐘 (毫秒)
  ...
  if (cachedDataStr) {
    try {
      ...
      if (now - cacheObj.timestamp < CACHE_TTL) {
        ...
        return; // 快取有效，提早結束函數，不向 GAS 發送請求
      } else {
...
function renderRadarChart(data) {
  ...
  window.myRadarChart = new Chart(ctx, {
    ...
    data: {
      labels: shortLabels,
      datasets: [{
        label: '參與次數', data: values, backgroundColor: 'rgba(52, 152, 219, 0.2)', borderColor: '#3498db', borderWidth: 2, pointBackgroundColor: ['#dc3545', '#28a745', '#f39c12', '#9b59b6'], pointBorderColor: '#fff', pointRadius: 6, pointHoverRadius: 8
      }]
    },
    options: {
      responsive: true, maintainAspectRatio: false,
      scales: { r: { startAngle: 0, min: 0, suggestedMax: 5, angleLines: { color: 'rgba(0, 0, 0, 0.15)' }, grid: { color: 'rgba(0, 0, 0, 0.1)' }, pointLabels: { font: { size: 14, family: '微軟正黑體', weight: 'bold' }, color: function(context) { const colors = ['#dc3545', '#28a745', '#f39c12', '#9b59b6']; return colors[context.index]; } }, ticks: { display: false, stepSize: 1 } } },
      plugins: { legend: { display: false }, tooltip: { callbacks: { title: function(tooltipItems) { return fullTooltips[tooltipItems[0].dataIndex]; }, label: function(context) { return `已參與：${context.raw} 次`; } } } }
    }
  });
}
...
function submitBinding() {
  ...
  if (!name || !phone || !birthday) {
    Swal.fire('提醒', '為了維護教會資料完整性，請完整填寫真實姓名、電話與生日喔！', 'warning'); return;
  }
  ...
  .then(res => {
    if(res.success) {
      clearCache(); // 【修改】成功後清除快取
      Swal.fire('綁定成功', res.message, 'success').then(() => { window.location.reload(); });
    } else {
      Swal.fire('綁定失敗', res.message, 'error'); btn.innerHTML = '<i class="fas fa-link"></i> 一鍵綁定 / 註冊';
    }
  })
  .catch(error => { Swal.fire('連線錯誤', error.message, 'error'); btn.innerHTML = '<i class="fas fa-link"></i> 一鍵綁定 / 註冊'; });
}
...
function saveProfile() {
  ...
  .then(res => {
    if(res.success) {
      ...
      Swal.fire('錯誤', res.message, 'error'); btn.innerHTML = '儲存修改';
    }
  })
  .catch(err => { Swal.fire('連線錯誤', err.message, 'error'); btn.innerHTML = '儲存修改'; });
}
...
function toggleFamilyView() { const isChecked = document.getElementById('familySwitch').checked; Swal.fire({ title: isChecked ? '已切換至【全戶視角】' : '已切換至【個人視角】', icon: 'success', toast: true, position: 'top-end', showConfirmButton: false, timer: 2000 }); }
...
function submitPrayer() {
  ...
  .then(res => {
    if (res.success) {
      ...
      Swal.fire('系統錯誤', res.message, 'error'); btn.innerHTML = '<i class="fas fa-paper-plane"></i> 重新送出';
    }
  })
  .catch(err => { Swal.fire('連線錯誤', err.message, 'error'); btn.innerHTML = '<i class="fas fa-paper-plane"></i> 重新送出'; });
}
...
function playPrayerAnimation(inputText) {
  ...
  textElement.classList.remove('anim-text'); doveIcon.classList.remove('anim-dove'); scripture.classList.remove('scripture-show');
...
function submitRegistration() {
  ...
  let pName = isProxy ? document.getElementById('regParticipantName').value : globalUserName;
  ...
  .then(res => {
    if (res.success) {
      ...
      bootstrap.Modal.getInstance(document.getElementById('regModal')).hide(); document.getElementById('regForm').reset(); 
    } else { Swal.fire('報名失敗', res.message, 'error'); btn.innerHTML = '<i class="fas fa-paper-plane"></i> 確定送出報名'; }
...
function cancelRegistration(regId, eventName) {
  Swal.fire({ title: '確定取消報名嗎？', html: `活動：<b>${eventName}</b><br>取消後將釋出您的名額。`, icon: 'warning', showCancelButton: true, confirmButtonColor: '#dc3545', cancelButtonColor: '#6c757d', confirmButtonText: '是的，我要取消', cancelButtonText: '保留名額' }).then((result) => {
    if (result.isConfirmed) {
      ...
      fetch(GAS_URL, { method: 'POST', body: JSON.stringify({ action: 'cancelRegistrationUser', regId: regId, uid: currentUID }) })
      .then(res => res.json()).then(res => {
        if (res.success) {
          clearCache(); // 【修改】成功後清除快取
          Swal.fire('已取消', res.message, 'success').then(() => { window.location.reload(); });
